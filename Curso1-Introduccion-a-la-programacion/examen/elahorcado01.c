...
bool letraIsInWord(char *inputVal, char *cad, char *adivina, int len, struct Player *P); //Nota abajo
void mostrarMisteryArray(char *array, int len);   //imprime cadena por adivinar
void printArray(char *letras, int len);   //imprime un array
...
int numAleatorio(int max);   //devuelve un numero aleatoria de 0 hasta max
...
void intro(struct Player *P1, struct Player *P2){
	...
	printf("*     Tienes 10 intentos para adivinar una palabra,     *\n");
...
void game(struct Player *P){
	...
	char letra[len];
	char letraUsada[10]={};
	int sitio = 0; //variable aumentara por uno para la entrada del array letraUsada 
	int i=10;
    	while(i>0){
      		mostrarMisteryArray(adivina, len);
		printf("       ¡Adivina una letra o apunta la solucion!\n");

		
		printArray(letraUsada, 10);
		...
		if(inword==false){
			...
			letraUsada[sitio]=*letra;
			sitio+=1;
			if(P->failed==7){
				printf("\n  ¡¡¡Estas muerto, muerto!!!\n");
				printf("\n   La palabra misteriosa era \"%s\". \n\n", sec);
				printf("\n      Hasta la próxima.\n\n");
				break;
			}
		}
...
void printArray(char *letrasUsadas, int len){

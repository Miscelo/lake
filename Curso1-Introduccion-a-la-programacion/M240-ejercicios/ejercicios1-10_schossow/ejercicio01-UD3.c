...
#define len 10                     // Numero de entradas en el array
...
int main(){
	...
	while(i!=len){     //En cada bucle, se adjunta una entrada al array num[]
		printf("%d: ", len-i);
	    	scanf("%d", &num[i]);
		i++;
	}
...
void printArray(int array[], int l){
	printf("\n Array:=[");
	int j;
	for(j=0;j<l;j++){   //bucle for para pasar al standard output stream cada entrada del array
		printf("%d", array[j]);
		(j<(l-1))? printf(",") : printf("]\n");
	}
...
// Info: numero primo, numero mayor 1, con dos divisores, 1 y su propio valor.
bool isprime(int n){
	if(n<=1){    //caso básico: todos los numeros enteros negativos, 0 y 1 no son numeros primos
		return false;
	}
	for(int i=2;i<n;i++){   // comienza por 1, todos los numeros naturales se dividen con 1.
		if(n%i == 0)          // comprobar, si la division con todos los numeros no tiene resto.
			return false;
	}
	return true;
}
...
void countPrimeNumbers(int array[], int l){
	int i;
	int primecounter = 0;
	for(i=0;i<l;i++){   //bucle, comprobar cada entrada en el array, si su valor es un numero primo.
